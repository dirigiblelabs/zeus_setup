...
function handleGetRequest(httpRequest, httpResponse, xss) {
    ...
    if (!hasConflictingParameters(id, count, metadata, httpResponse)) {
        if (id) {
            var entity = applicationsDao.get(id);
            if (entity !== null) {
                sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(entity, null, 2));
            } else {
                sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'application_id\'=' + id);
            }
        } else if (count !== null) {
            var applicationsCount = applicationsDao.count();
            sendResponse(httpResponse, httpResponse.OK, 'text/plain', applicationsCount);
        } else if (metadata !== null) {
            var applicationsMetadata = applicationsDao.metadata();
            sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(applicationsMetadata, null, 2));
        } else {
            var applications = applicationsDao.list(limit, offset, sort, desc);
            sendResponse(httpResponse, httpResponse.OK, 'application/json', JSON.stringify(applications, null, 2));
        }
    }
}
...
function handlePostRequest(httpRequest, httpResponse) {
    ...
    var id = applicationsDao.create(entity);
...
function handlePutRequest(httpRequest, httpResponse) {
    var entity = getRequestBody(httpRequest);
    var id = getIdParameter(httpRequest, xss);
    id = id !== null ? id : entity.application_id;
    if (id !== null) {
        if (applicationsDao.get(id) !== null) {
            applicationsDao.update(entity);
            sendResponse(httpResponse, httpResponse.NO_CONTENT);
        } else {
            sendResponse(httpResponse, httpResponse.NOT_FOUND, 'text/plain', 'No entity found with \'application_id\'=' + id);
        }
    } else {
        sendResponse(httpResponse, httpResponse.PRECONDITION_FAILED, 'text/plain', 'Expected \'application_id\' parameter is missing!');
    }
...
function getIdParameter(httpRequest, xss) {
    ...
    id = id !== null ? id : xss.escapeSql(httpRequest.getParameter('application_id'));
    return id;
}
